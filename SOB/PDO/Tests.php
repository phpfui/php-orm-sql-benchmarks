...
class Tests extends \SOB\Test
	...
	public function delete(int $id) : bool
		{
		return $this->pdo->execute('delete from employee where employee_id=?', [$id]);
		}
	...
	public function insert(int $id) : int
		{
		$this->pdo->execute('insert into employee (company, last_name, first_name) values (?,?,?)', ["Company {$id}", "Last {$id}", "First {$id}"]);

		$name = 'employee';
		if ($this->pdo->postGre)
			{
			$name .= '_employee_id_seq';
			}
	...
	public function read(int $id) : ?object
		{
		$result = $this->pdo->getRow('select * from employee where employee_id=?', [$id]);
	...
	public function update(int $id, int $to) : bool
		{
		return $this->pdo->execute('update employee set last_name=? where employee_id=?', [ "Updated {$to}", $id]);
		}
	}
