...
abstract class Customer implements ActiveRecordInterface
{
	 ...
	 * @var string
	 */
	public const TABLE_MAP = '\\SOB\\Propel2\\Map\\CustomerTableMap';
	...
	/**
	 * Initializes internal state of SOB\Propel2\Base\Customer object.
	 */
	...
	public function buildCriteria() : Criteria
	{
		...
		if ($this->isColumnModified(CustomerTableMap::COL_EMAIL_ADDRESS)) {
			$criteria->add(CustomerTableMap::COL_EMAIL_ADDRESS, $this->email_address);
		}

		if ($this->isColumnModified(CustomerTableMap::COL_JOB_TITLE)) {
			$criteria->add(CustomerTableMap::COL_JOB_TITLE, $this->job_title);
		}

		if ($this->isColumnModified(CustomerTableMap::COL_BUSINESS_PHONE)) {
			$criteria->add(CustomerTableMap::COL_BUSINESS_PHONE, $this->business_phone);
		}

		if ($this->isColumnModified(CustomerTableMap::COL_HOME_PHONE)) {
			$criteria->add(CustomerTableMap::COL_HOME_PHONE, $this->home_phone);
		}

		if ($this->isColumnModified(CustomerTableMap::COL_MOBILE_PHONE)) {
			$criteria->add(CustomerTableMap::COL_MOBILE_PHONE, $this->mobile_phone);
		}

		if ($this->isColumnModified(CustomerTableMap::COL_FAX_NUMBER)) {
			$criteria->add(CustomerTableMap::COL_FAX_NUMBER, $this->fax_number);
		}
		...
		if ($this->isColumnModified(CustomerTableMap::COL_STATE_PROVINCE)) {
			$criteria->add(CustomerTableMap::COL_STATE_PROVINCE, $this->state_province);
		}

		if ($this->isColumnModified(CustomerTableMap::COL_ZIP_POSTAL_CODE)) {
			$criteria->add(CustomerTableMap::COL_ZIP_POSTAL_CODE, $this->zip_postal_code);
		}

		if ($this->isColumnModified(CustomerTableMap::COL_COUNTRY_REGION)) {
			$criteria->add(CustomerTableMap::COL_COUNTRY_REGION, $this->country_region);
		}

		if ($this->isColumnModified(CustomerTableMap::COL_WEB_PAGE)) {
			$criteria->add(CustomerTableMap::COL_WEB_PAGE, $this->web_page);
		}

		if ($this->isColumnModified(CustomerTableMap::COL_NOTES)) {
			$criteria->add(CustomerTableMap::COL_NOTES, $this->notes);
		}

		if ($this->isColumnModified(CustomerTableMap::COL_ATTACHMENTS)) {
			$criteria->add(CustomerTableMap::COL_ATTACHMENTS, $this->attachments);
		}
	 ...
	 * @return \SOB\Propel2\Customer Clone of current object.
	 */
	 ...
	 * objects.
	 *
	 * @param object $copyObj An object of \SOB\Propel2\Customer (or compatible) type.
	 * @param bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	...
	public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM) : int
	{
		try {
			...
			$col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : CustomerTableMap::translateFieldName('Company', TableMap::TYPE_PHPNAME, $indexType)];
			...
			$col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : CustomerTableMap::translateFieldName('EmailAddress', TableMap::TYPE_PHPNAME, $indexType)];
			$this->email_address = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : CustomerTableMap::translateFieldName('JobTitle', TableMap::TYPE_PHPNAME, $indexType)];
			$this->job_title = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : CustomerTableMap::translateFieldName('BusinessPhone', TableMap::TYPE_PHPNAME, $indexType)];
			$this->business_phone = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 7 + $startcol : CustomerTableMap::translateFieldName('HomePhone', TableMap::TYPE_PHPNAME, $indexType)];
			$this->home_phone = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 8 + $startcol : CustomerTableMap::translateFieldName('MobilePhone', TableMap::TYPE_PHPNAME, $indexType)];
			$this->mobile_phone = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 9 + $startcol : CustomerTableMap::translateFieldName('FaxNumber', TableMap::TYPE_PHPNAME, $indexType)];
			$this->fax_number = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 10 + $startcol : CustomerTableMap::translateFieldName('Address', TableMap::TYPE_PHPNAME, $indexType)];
			$this->address = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 11 + $startcol : CustomerTableMap::translateFieldName('City', TableMap::TYPE_PHPNAME, $indexType)];
			$this->city = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 12 + $startcol : CustomerTableMap::translateFieldName('StateProvince', TableMap::TYPE_PHPNAME, $indexType)];
			$this->state_province = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 13 + $startcol : CustomerTableMap::translateFieldName('ZipPostalCode', TableMap::TYPE_PHPNAME, $indexType)];
			$this->zip_postal_code = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 14 + $startcol : CustomerTableMap::translateFieldName('CountryRegion', TableMap::TYPE_PHPNAME, $indexType)];
			$this->country_region = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 15 + $startcol : CustomerTableMap::translateFieldName('WebPage', TableMap::TYPE_PHPNAME, $indexType)];
			$this->web_page = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 16 + $startcol : CustomerTableMap::translateFieldName('Notes', TableMap::TYPE_PHPNAME, $indexType)];
			$this->notes = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 17 + $startcol : CustomerTableMap::translateFieldName('Attachments', TableMap::TYPE_PHPNAME, $indexType)];
			...
			throw new PropelException(\sprintf('Error populating %s object', '\\SOB\\Propel2\\Customer'), 0, $e);
		}
	}
	...
	public function setAttachments($v)
	{
		...
		$this->modifiedColumns[CustomerTableMap::COL_ATTACHMENTS] = true;
	...
	public function setBusinessPhone($v)
	{
		...
		if ($this->business_phone !== $v) {
			...
			$this->modifiedColumns[CustomerTableMap::COL_BUSINESS_PHONE] = true;
		}
	...
	public function setCountryRegion($v)
	{
		...
		if ($this->country_region !== $v) {
			...
			$this->modifiedColumns[CustomerTableMap::COL_COUNTRY_REGION] = true;
		}
	...
	public function setEmailAddress($v)
	{
		...
		if ($this->email_address !== $v) {
			...
			$this->modifiedColumns[CustomerTableMap::COL_EMAIL_ADDRESS] = true;
		}
	...
	public function setFaxNumber($v)
	{
		...
		if ($this->fax_number !== $v) {
			$this->fax_number = $v;
			$this->modifiedColumns[CustomerTableMap::COL_FAX_NUMBER] = true;
		}
	...
	public function setHomePhone($v)
	{
		...
		if ($this->home_phone !== $v) {
			$this->home_phone = $v;
			$this->modifiedColumns[CustomerTableMap::COL_HOME_PHONE] = true;
		}
	...
	public function setJobTitle($v)
	{
		...
		if ($this->job_title !== $v) {
			$this->job_title = $v;
			$this->modifiedColumns[CustomerTableMap::COL_JOB_TITLE] = true;
		}
	...
	public function setMobilePhone($v)
	{
		...
		if ($this->mobile_phone !== $v) {
			$this->mobile_phone = $v;
			$this->modifiedColumns[CustomerTableMap::COL_MOBILE_PHONE] = true;
		}
	...
	public function setNotes($v)
	{
		...
		if ($this->notes !== $v) {
			$this->notes = $v;
			$this->modifiedColumns[CustomerTableMap::COL_NOTES] = true;
		}
	...
	public function setStateProvince($v)
	{
		...
		if ($this->state_province !== $v) {
			...
			$this->modifiedColumns[CustomerTableMap::COL_STATE_PROVINCE] = true;
		}
	...
	public function setWebPage($v)
	{
		...
		if ($this->web_page !== $v) {
			$this->web_page = $v;
			$this->modifiedColumns[CustomerTableMap::COL_WEB_PAGE] = true;
		}
	...
	public function setZipPostalCode($v)
	{
		...
		if ($this->zip_postal_code !== $v) {
			...
			$this->modifiedColumns[CustomerTableMap::COL_ZIP_POSTAL_CODE] = true;
		}
