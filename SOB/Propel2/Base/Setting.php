...
abstract class Setting implements ActiveRecordInterface
{
	 ...
	 * @var string
	 */
	public const TABLE_MAP = '\\SOB\\Propel2\\Map\\SettingTableMap';
	...
	/**
	 * The value for the setting_data field.
	 *
	 * @var        string|null
	 */
	...
	/**
	 * Initializes internal state of SOB\Propel2\Base\Setting object.
	 */
	...
	public function buildCriteria() : Criteria
	{
		...
		if ($this->isColumnModified(SettingTableMap::COL_SETTING_ID)) {
			$criteria->add(SettingTableMap::COL_SETTING_ID, $this->setting_id);
		}

		if ($this->isColumnModified(SettingTableMap::COL_SETTING_DATA)) {
			$criteria->add(SettingTableMap::COL_SETTING_DATA, $this->setting_data);
		}
	 ...
	 * @return \SOB\Propel2\Setting Clone of current object.
	 */
	 ...
	 * objects.
	 *
	 * @param object $copyObj An object of \SOB\Propel2\Setting (or compatible) type.
	 * @param bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	...
	{
		$copyObj->setSettingData($this->getSettingData());
	...
	public function fromArray(array $arr, string $keyType = TableMap::TYPE_PHPNAME)
	{
		...
		if (\array_key_exists($keys[1], $arr)) {
			$this->setSettingData($arr[$keys[1]]);
		}
	...
	/**
	 * Get the [setting_data] column value.
	 *
	 * @return string|null
	 */
	...
	public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM) : int
	{
		try {
			...
			$col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : SettingTableMap::translateFieldName('SettingId', TableMap::TYPE_PHPNAME, $indexType)];
			$this->setting_id = (null !== $col) ? (int)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : SettingTableMap::translateFieldName('SettingData', TableMap::TYPE_PHPNAME, $indexType)];
			$this->setting_data = (null !== $col) ? (string)$col : null;
			...
			return $startcol + 2; // 2 = SettingTableMap::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException(\sprintf('Error populating %s object', '\\SOB\\Propel2\\Setting'), 0, $e);
		}
	}
	...
	/**
	 * Set the value of [setting_data] column.
	 *
	 * @param string|null $v New value
	 * @return $this The current object (for fluent API support)
	 */
	public function setSettingData($v)
	{
		if (null !== $v) {
			$v = (string)$v;
		}

		if ($this->setting_data !== $v) {
			$this->setting_data = $v;
			$this->modifiedColumns[SettingTableMap::COL_SETTING_DATA] = true;
		}
	...
	public function setSettingId($v)
	{
		...
		if ($this->setting_id !== $v) {
			$this->setting_id = $v;
			$this->modifiedColumns[SettingTableMap::COL_SETTING_ID] = true;
		}
	...
	protected function doInsert(ConnectionInterface $con) : void
	{
		...
		if ($this->isColumnModified(SettingTableMap::COL_SETTING_DATA)) {
			$modifiedColumns[':p' . $index++] = 'setting_data';
		}
		...
		try {
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					...
					case 'setting_data':
						$stmt->bindValue($identifier, $this->setting_data, PDO::PARAM_STR);
