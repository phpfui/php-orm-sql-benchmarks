...
abstract class Employee implements ActiveRecordInterface
{
	 ...
	 * @var string
	 */
	public const TABLE_MAP = '\\SOB\\Propel2\\Map\\EmployeeTableMap';
	...
	/**
	 * Initializes internal state of SOB\Propel2\Base\Employee object.
	 */
	...
	public function buildCriteria() : Criteria
	{
		...
		if ($this->isColumnModified(EmployeeTableMap::COL_COMPANY)) {
			$criteria->add(EmployeeTableMap::COL_COMPANY, $this->company);
		}
		...
		if ($this->isColumnModified(EmployeeTableMap::COL_EMAIL_ADDRESS)) {
			$criteria->add(EmployeeTableMap::COL_EMAIL_ADDRESS, $this->email_address);
		}
		...
		if ($this->isColumnModified(EmployeeTableMap::COL_BUSINESS_PHONE)) {
			$criteria->add(EmployeeTableMap::COL_BUSINESS_PHONE, $this->business_phone);
		}

		if ($this->isColumnModified(EmployeeTableMap::COL_HOME_PHONE)) {
			$criteria->add(EmployeeTableMap::COL_HOME_PHONE, $this->home_phone);
		}

		if ($this->isColumnModified(EmployeeTableMap::COL_MOBILE_PHONE)) {
			$criteria->add(EmployeeTableMap::COL_MOBILE_PHONE, $this->mobile_phone);
		}

		if ($this->isColumnModified(EmployeeTableMap::COL_FAX_NUMBER)) {
			$criteria->add(EmployeeTableMap::COL_FAX_NUMBER, $this->fax_number);
		}
		...
		if ($this->isColumnModified(EmployeeTableMap::COL_STATE_PROVINCE)) {
			$criteria->add(EmployeeTableMap::COL_STATE_PROVINCE, $this->state_province);
		}

		if ($this->isColumnModified(EmployeeTableMap::COL_ZIP_POSTAL_CODE)) {
			$criteria->add(EmployeeTableMap::COL_ZIP_POSTAL_CODE, $this->zip_postal_code);
		}

		if ($this->isColumnModified(EmployeeTableMap::COL_COUNTRY_REGION)) {
			$criteria->add(EmployeeTableMap::COL_COUNTRY_REGION, $this->country_region);
		}

		if ($this->isColumnModified(EmployeeTableMap::COL_WEB_PAGE)) {
			$criteria->add(EmployeeTableMap::COL_WEB_PAGE, $this->web_page);
		}

		if ($this->isColumnModified(EmployeeTableMap::COL_NOTES)) {
			$criteria->add(EmployeeTableMap::COL_NOTES, $this->notes);
		}

		if ($this->isColumnModified(EmployeeTableMap::COL_ATTACHMENTS)) {
			$criteria->add(EmployeeTableMap::COL_ATTACHMENTS, $this->attachments);
		}
	 ...
	 * @return \SOB\Propel2\Employee Clone of current object.
	 */
	 ...
	 * objects.
	 *
	 * @param object $copyObj An object of \SOB\Propel2\Employee (or compatible) type.
	 * @param bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	...
	public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM) : int
	{
		try {
			...
			$col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : EmployeeTableMap::translateFieldName('Company', TableMap::TYPE_PHPNAME, $indexType)];
			...
			$col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : EmployeeTableMap::translateFieldName('FirstName', TableMap::TYPE_PHPNAME, $indexType)];
			$this->first_name = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : EmployeeTableMap::translateFieldName('EmailAddress', TableMap::TYPE_PHPNAME, $indexType)];
			...
			$col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : EmployeeTableMap::translateFieldName('BusinessPhone', TableMap::TYPE_PHPNAME, $indexType)];
			$this->business_phone = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 7 + $startcol : EmployeeTableMap::translateFieldName('HomePhone', TableMap::TYPE_PHPNAME, $indexType)];
			$this->home_phone = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 8 + $startcol : EmployeeTableMap::translateFieldName('MobilePhone', TableMap::TYPE_PHPNAME, $indexType)];
			$this->mobile_phone = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 9 + $startcol : EmployeeTableMap::translateFieldName('FaxNumber', TableMap::TYPE_PHPNAME, $indexType)];
			$this->fax_number = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 10 + $startcol : EmployeeTableMap::translateFieldName('Address', TableMap::TYPE_PHPNAME, $indexType)];
			$this->address = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 11 + $startcol : EmployeeTableMap::translateFieldName('City', TableMap::TYPE_PHPNAME, $indexType)];
			$this->city = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 12 + $startcol : EmployeeTableMap::translateFieldName('StateProvince', TableMap::TYPE_PHPNAME, $indexType)];
			$this->state_province = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 13 + $startcol : EmployeeTableMap::translateFieldName('ZipPostalCode', TableMap::TYPE_PHPNAME, $indexType)];
			$this->zip_postal_code = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 14 + $startcol : EmployeeTableMap::translateFieldName('CountryRegion', TableMap::TYPE_PHPNAME, $indexType)];
			$this->country_region = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 15 + $startcol : EmployeeTableMap::translateFieldName('WebPage', TableMap::TYPE_PHPNAME, $indexType)];
			$this->web_page = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 16 + $startcol : EmployeeTableMap::translateFieldName('Notes', TableMap::TYPE_PHPNAME, $indexType)];
			$this->notes = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 17 + $startcol : EmployeeTableMap::translateFieldName('Attachments', TableMap::TYPE_PHPNAME, $indexType)];
			...
			return $startcol + 18; // 18 = EmployeeTableMap::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException(\sprintf('Error populating %s object', '\\SOB\\Propel2\\Employee'), 0, $e);
		}
	}
	...
	public function setAttachments($v)
	{
		...
		$this->modifiedColumns[EmployeeTableMap::COL_ATTACHMENTS] = true;
	...
	public function setBusinessPhone($v)
	{
		...
		if ($this->business_phone !== $v) {
			...
			$this->modifiedColumns[EmployeeTableMap::COL_BUSINESS_PHONE] = true;
		}
	...
	public function setCompany($v)
	{
		...
		if ($this->company !== $v) {
			$this->company = $v;
			$this->modifiedColumns[EmployeeTableMap::COL_COMPANY] = true;
		}
	...
	public function setCountryRegion($v)
	{
		...
		if ($this->country_region !== $v) {
			...
			$this->modifiedColumns[EmployeeTableMap::COL_COUNTRY_REGION] = true;
		}
	...
	public function setEmailAddress($v)
	{
		...
		if ($this->email_address !== $v) {
			...
			$this->modifiedColumns[EmployeeTableMap::COL_EMAIL_ADDRESS] = true;
		}
	...
	public function setFaxNumber($v)
	{
		...
		if ($this->fax_number !== $v) {
			$this->fax_number = $v;
			$this->modifiedColumns[EmployeeTableMap::COL_FAX_NUMBER] = true;
		}
	...
	public function setHomePhone($v)
	{
		...
		if ($this->home_phone !== $v) {
			$this->home_phone = $v;
			$this->modifiedColumns[EmployeeTableMap::COL_HOME_PHONE] = true;
		}
	...
	public function setMobilePhone($v)
	{
		...
		if ($this->mobile_phone !== $v) {
			$this->mobile_phone = $v;
			$this->modifiedColumns[EmployeeTableMap::COL_MOBILE_PHONE] = true;
		}
	...
	public function setNotes($v)
	{
		...
		if ($this->notes !== $v) {
			$this->notes = $v;
			$this->modifiedColumns[EmployeeTableMap::COL_NOTES] = true;
		}
	...
	public function setStateProvince($v)
	{
		...
		if ($this->state_province !== $v) {
			...
			$this->modifiedColumns[EmployeeTableMap::COL_STATE_PROVINCE] = true;
		}
	...
	public function setWebPage($v)
	{
		...
		if ($this->web_page !== $v) {
			$this->web_page = $v;
			$this->modifiedColumns[EmployeeTableMap::COL_WEB_PAGE] = true;
		}
	...
	public function setZipPostalCode($v)
	{
		...
		if ($this->zip_postal_code !== $v) {
			...
			$this->modifiedColumns[EmployeeTableMap::COL_ZIP_POSTAL_CODE] = true;
		}
