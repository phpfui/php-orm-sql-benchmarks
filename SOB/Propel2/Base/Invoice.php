...
abstract class Invoice implements ActiveRecordInterface
{
	 ...
	 * @var string
	 */
	public const TABLE_MAP = '\\SOB\\Propel2\\Map\\InvoiceTableMap';
	...
	/**
	 * Initializes internal state of SOB\Propel2\Base\Invoice object.
	 * @see applyDefaults()
	 */
	...
	{
		$this->tax = '0.0000';
		$this->shipping = '0.0000';
		$this->amount_due = '0.0000';
	}
	...
	public function buildCriteria() : Criteria
	{
		...
		if ($this->isColumnModified(InvoiceTableMap::COL_ORDER_ID)) {
			$criteria->add(InvoiceTableMap::COL_ORDER_ID, $this->order_id);
		}
		...
		if ($this->isColumnModified(InvoiceTableMap::COL_DUE_DATE)) {
			$criteria->add(InvoiceTableMap::COL_DUE_DATE, $this->due_date);
		}

		if ($this->isColumnModified(InvoiceTableMap::COL_TAX)) {
			$criteria->add(InvoiceTableMap::COL_TAX, $this->tax);
		}

		if ($this->isColumnModified(InvoiceTableMap::COL_SHIPPING)) {
			$criteria->add(InvoiceTableMap::COL_SHIPPING, $this->shipping);
		}

		if ($this->isColumnModified(InvoiceTableMap::COL_AMOUNT_DUE)) {
			$criteria->add(InvoiceTableMap::COL_AMOUNT_DUE, $this->amount_due);
		}
	 ...
	 * @return \SOB\Propel2\Invoice Clone of current object.
	 */
	 ...
	 * objects.
	 *
	 * @param object $copyObj An object of \SOB\Propel2\Invoice (or compatible) type.
	 * @param bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	...
	public function fromArray(array $arr, string $keyType = TableMap::TYPE_PHPNAME)
	{
		...
		if (\array_key_exists($keys[4], $arr)) {
			$this->setTax($arr[$keys[4]]);
		}

		if (\array_key_exists($keys[5], $arr)) {
			$this->setShipping($arr[$keys[5]]);
		}

		if (\array_key_exists($keys[6], $arr)) {
			$this->setAmountDue($arr[$keys[6]]);
		}
	...
	public function hasOnlyDefaultValues() : bool
	{
			...
			if ('0.0000' !== $this->shipping) {
				return false;
			}

			return ! ('0.0000' !== $this->amount_due);
	...
	public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM) : int
	{
		try {
			...
			$col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : InvoiceTableMap::translateFieldName('OrderId', TableMap::TYPE_PHPNAME, $indexType)];
			...
			$col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : InvoiceTableMap::translateFieldName('DueDate', TableMap::TYPE_PHPNAME, $indexType)];
			...
			$col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : InvoiceTableMap::translateFieldName('Tax', TableMap::TYPE_PHPNAME, $indexType)];
			$this->tax = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : InvoiceTableMap::translateFieldName('Shipping', TableMap::TYPE_PHPNAME, $indexType)];
			$this->shipping = (null !== $col) ? (string)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : InvoiceTableMap::translateFieldName('AmountDue', TableMap::TYPE_PHPNAME, $indexType)];
			...
			throw new PropelException(\sprintf('Error populating %s object', '\\SOB\\Propel2\\Invoice'), 0, $e);
		}
	}
	...
	public function setAmountDue($v)
	{
		...
		if ($this->amount_due !== $v) {
			$this->amount_due = $v;
			$this->modifiedColumns[InvoiceTableMap::COL_AMOUNT_DUE] = true;
		}
	...
	public function setDueDate($v)
	{
		...
		if (null !== $this->due_date || null !== $dt) {
			if (null === $this->due_date || null === $dt || $dt->format('Y-m-d H:i:s.u') !== $this->due_date->format('Y-m-d H:i:s.u')) {
				$this->due_date = null === $dt ? null : clone $dt;
				$this->modifiedColumns[InvoiceTableMap::COL_DUE_DATE] = true;
			}
	...
	public function setInvoiceDate($v)
	{
		...
		if (null !== $this->invoice_date || null !== $dt) {
			if (null === $this->invoice_date || null === $dt || $dt->format('Y-m-d H:i:s.u') !== $this->invoice_date->format('Y-m-d H:i:s.u')) {
				$this->invoice_date = null === $dt ? null : clone $dt;
	...
	public function setShipping($v)
	{
		...
		if ($this->shipping !== $v) {
			$this->shipping = $v;
			$this->modifiedColumns[InvoiceTableMap::COL_SHIPPING] = true;
		}
	...
	public function setTax($v)
	{
		...
		if ($this->tax !== $v) {
			$this->tax = $v;
			$this->modifiedColumns[InvoiceTableMap::COL_TAX] = true;
		}
	...
	public function toArray(string $keyType = TableMap::TYPE_PHPNAME, bool $includeLazyLoadColumns = true, array $alreadyDumpedObjects = []) : array
	{
		...
		$result = [
			...
			$keys[4] => $this->getTax(),
			$keys[5] => $this->getShipping(),
			$keys[6] => $this->getAmountDue(),
		];
