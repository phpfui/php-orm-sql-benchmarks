<?php

namespace SOB\PHPFUI\Record\Definition;

/**
 * Autogenerated. Do not modify. Modify SQL table, then generate with \PHPFUI\ORM\Tool\Generate\CRUD class.
 *
 * @property ?string $date_allocated MySQL type timestamp with time zone
 * @property float $discount MySQL type double precision
 * @property ?int $inventory_transaction_id MySQL type integer
 * @property \SOB\PHPFUI\Record\InventoryTransaction $inventory_transaction related record
 * @property int $order_detail_id MySQL type integer
 * @property \SOB\PHPFUI\Record\OrderDetail $order_detail related record
 * @property ?int $order_detail_status_id MySQL type integer
 * @property \SOB\PHPFUI\Record\OrderDetailStatus $order_detail_status related record
 * @property int $order_id MySQL type integer
 * @property \SOB\PHPFUI\Record\Order $order related record
 * @property ?int $product_id MySQL type integer
 * @property \SOB\PHPFUI\Record\Product $product related record
 * @property ?int $purchase_order_id MySQL type integer
 * @property \SOB\PHPFUI\Record\PurchaseOrder $purchase_order related record
 * @property float $quantity MySQL type numeric
 * @property ?float $unit_price MySQL type numeric
 */
abstract class OrderDetail extends \PHPFUI\ORM\Record
	{
	protected static bool $autoIncrement = true;

	/** @var array<string, \PHPFUI\ORM\FieldDefinition> */
	protected static array $fields = [];

	/** @var array<string> */
	protected static array $primaryKeys = ['order_detail_id', ];

	protected static string $table = 'order_detail';

	public function initFieldDefinitions() : static
		{
		if (! \count(static::$fields))
			{
			static::$fields = [
				'date_allocated' => new \PHPFUI\ORM\FieldDefinition('timestamp with time zone', 'string', 0, true, ),
				'discount' => new \PHPFUI\ORM\FieldDefinition('double precision', 'float', 0, false, 0, ),
				'inventory_transaction_id' => new \PHPFUI\ORM\FieldDefinition('integer', 'int', 0, true, ),
				'order_detail_id' => new \PHPFUI\ORM\FieldDefinition('integer', 'int', 0, false, ),
				'order_detail_status_id' => new \PHPFUI\ORM\FieldDefinition('integer', 'int', 0, true, ),
				'order_id' => new \PHPFUI\ORM\FieldDefinition('integer', 'int', 0, false, ),
				'product_id' => new \PHPFUI\ORM\FieldDefinition('integer', 'int', 0, true, ),
				'purchase_order_id' => new \PHPFUI\ORM\FieldDefinition('integer', 'int', 0, true, ),
				'quantity' => new \PHPFUI\ORM\FieldDefinition('numeric', 'float', 0, false, 0.0000, ),
				'unit_price' => new \PHPFUI\ORM\FieldDefinition('numeric', 'float', 0, true, 0.0000, ),
			];
			}

		return $this;
		}
	}
