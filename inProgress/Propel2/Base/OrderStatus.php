...
abstract class OrderStatus implements ActiveRecordInterface
{
	 ...
	 * @var string
	 */
	public const TABLE_MAP = '\\SOB\\Propel2\\Map\\OrderStatusTableMap';
	...
	/**
	 * The value for the order_status_name field.
	 *
	 * @var        string
	 */
	...
	/**
	 * Initializes internal state of SOB\Propel2\Base\OrderStatus object.
	 */
	...
	public function buildCriteria() : Criteria
	{
		...
		if ($this->isColumnModified(OrderStatusTableMap::COL_ORDER_STATUS_ID)) {
			$criteria->add(OrderStatusTableMap::COL_ORDER_STATUS_ID, $this->order_status_id);
		}

		if ($this->isColumnModified(OrderStatusTableMap::COL_ORDER_STATUS_NAME)) {
			$criteria->add(OrderStatusTableMap::COL_ORDER_STATUS_NAME, $this->order_status_name);
		}
	 ...
	 * @return \SOB\Propel2\OrderStatus Clone of current object.
	 */
	 ...
	 * objects.
	 *
	 * @param object $copyObj An object of \SOB\Propel2\OrderStatus (or compatible) type.
	 * @param bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	...
	public function copyInto(object $copyObj, bool $deepCopy = false, bool $makeNew = true) : void
	{
		...
		$copyObj->setOrderStatusName($this->getOrderStatusName());
	...
	public function fromArray(array $arr, string $keyType = TableMap::TYPE_PHPNAME)
	{
		...
		if (\array_key_exists($keys[1], $arr)) {
			$this->setOrderStatusName($arr[$keys[1]]);
		}
	...
	public function getByPosition(int $pos)
	{
		switch ($pos) {
			...
			case 1:
				return $this->getOrderStatusName();
	...
	/**
	 * Get the [order_status_name] column value.
	 *
	 * @return string
	 */
	...
	public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM) : int
	{
		try {
			...
			$col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : OrderStatusTableMap::translateFieldName('OrderStatusId', TableMap::TYPE_PHPNAME, $indexType)];
			$this->order_status_id = (null !== $col) ? (int)$col : null;

			$col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : OrderStatusTableMap::translateFieldName('OrderStatusName', TableMap::TYPE_PHPNAME, $indexType)];
			$this->order_status_name = (null !== $col) ? (string)$col : null;
			...
			throw new PropelException(\sprintf('Error populating %s object', '\\SOB\\Propel2\\OrderStatus'), 0, $e);
		}
	}
	...
	{
		return null === $this->getOrderStatusId();
	}
	...
	public function setOrderStatusId($v)
	{
		...
		if ($this->order_status_id !== $v) {
			...
			$this->modifiedColumns[OrderStatusTableMap::COL_ORDER_STATUS_ID] = true;
		}
	...
	/**
	 * Set the value of [order_status_name] column.
	 *
	 * @param string $v New value
	 * @return $this The current object (for fluent API support)
	 */
	public function setOrderStatusName($v)
	{
		if (null !== $v) {
			$v = (string)$v;
		}

		if ($this->order_status_name !== $v) {
			$this->order_status_name = $v;
			$this->modifiedColumns[OrderStatusTableMap::COL_ORDER_STATUS_NAME] = true;
		}
	...
	/**
	 * Generic method to set the primary key (order_status_id column).
	 *
	 * @param int|null $key Primary key.
	 */
	public function setPrimaryKey(?int $key = null) : void
	{
		$this->setOrderStatusId($key);
	}
	...
	public function toArray(string $keyType = TableMap::TYPE_PHPNAME, bool $includeLazyLoadColumns = true, array $alreadyDumpedObjects = []) : array
	{
		...
		$result = [
			...
			$keys[1] => $this->getOrderStatusName(),
		];
	...
	protected function doInsert(ConnectionInterface $con) : void
	{
		...
		if ($this->isColumnModified(OrderStatusTableMap::COL_ORDER_STATUS_NAME)) {
			$modifiedColumns[':p' . $index++] = 'order_status_name';
		}
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					...
					case 'order_status_name':
						$stmt->bindValue($identifier, $this->order_status_name, PDO::PARAM_STR);
